...
static bool checkdupSym(const char *sym) {
    ...
    if (is_dup) {
        ...
        CTI(CTI_ERROR, true, "redifinition of symbol '%s'\n", sym);
        CCNerrorAction();
    }
...
node_st *INITSYMTABLES_globaldec(node_st *node) {
    ...
    symtable_insert(peekSymtab(), GLOBALDEC_ID(node), ent);
    return node;
}
...
node_st *INITSYMTABLES_globaldef(node_st *node) {
    ...
    symtable_insert(peekSymtab(), GLOBALDEF_ID(node), ent);
    return node;
}
...
node_st *INITSYMTABLES_voidfunheader(node_st *node) {
    ...
    for (node_st *params = VOIDFUNHEADER_PARAMS(node); params; params = HEADERPARAMS_NEXT(params)) {
        ...
        symtable_insert(peekSymtab(), PARAMETER_ID(param), ent);
    }
    ...
    symtable_insert(peekSymtab(), VOIDFUNHEADER_ID(node), ent);
...
node_st *INITSYMTABLES_basicfunheader(node_st *node) {
    ...
    symtable_insert(peekSymtab(), BASICFUNHEADER_ID(node), ent);
...
node_st *INITSYMTABLES_funbody(node_st *node) {
    ...
    for (node_st *vardecs = FUNBODY_VARDECS(node); vardecs; vardecs = VARDECS_NEXT(vardecs)) {
        ...
        symtable_insert(peekSymtab(), VARDEC_ID(vardec), ent);
    }
